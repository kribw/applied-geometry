...
class PSimpleSubSurf : public GMlib::PSurf<T, 3> {
  ...
  protected:
    ...
    GMlib::Vector<float, 3>
      _trans;   // Translation so center is origin in local coordinate system
...
template <typename T>
inline PSimpleSubSurf<T>::PSimpleSubSurf(GMlib::PSurf<T, 3>* s, T su, T eu,
                                         T sv, T ev)
{
...
template <typename T>
inline PSimpleSubSurf<T>::PSimpleSubSurf(PSurf<T, 3>* s, T su, T eu, T u, T sv,
                                         T ev, T v)
{
...
template <typename T>
inline PSimpleSubSurf<T>::PSimpleSubSurf(const PSimpleSubSurf<T>& copy)
...
template <typename T>
void PSimpleSubSurf<T>::eval(T u, T v, int d1, int d2, bool /*lu*/,
                             bool /*lv*/) const
{
...
template <typename T>
inline void PSimpleSubSurf<T>::set(GMlib::PSurf<T, 3>* s, T su, T eu, T u, T sv,
                                   T ev, T v)
{
    _s  = s;
    _su = su;
